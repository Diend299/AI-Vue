    ...
    <main class="layout">
      <el-card class="panel">
        ...
        <el-form label-position="top">
          ...
          <!-- LoRA强度 -->
          <el-form-item label="LoRA 强度" v-if="loraModel">
            <div class="lora-strength-control">
              <el-slider 
                ...
                show-stops
              />
              <span class="strength-value">{{ loraStrength.toFixed(2) }}</span>
            </div>
          </el-form-item>
...
// LoRA相关
const loraModel = ref('') // 选中的LoRA模型
const loraStrength = ref(0.75) // LoRA强度，默认0.75
const loraOptions = ref<LoRAOption[]>([]) // LoRA选项列表
const loraLoading = ref(false) // 加载状态
...
async function handleSubmitTask() {
  ...
  try {
    ...
    // 添加LoRA参数
    if (loraModel.value) {
      formData.append('loraModel', loraModel.value)
      formData.append('loraStrength', String(loraStrength.value))
    }
...
.lora-strength-control .el-slider {
  flex: 1;
}

.lora-strength-control .strength-value {
  min-width: 48px;
  text-align: center;
  font-size: 14px;
  font-weight: 600;
...
.panel :deep(.el-slider__bar) {
  background-color: #6a8aff;
}

.panel :deep(.el-slider__button) {
  border-color: #6a8aff;
  background-color: #6a8aff;
}

.panel :deep(.el-slider__stop) {
